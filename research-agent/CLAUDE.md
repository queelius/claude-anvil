...
- The agent uses `model: inherit`, so it runs on whatever model the session selected. Choose Fable for the longest runs with `/model fable`; the 1M context window comes with a Fable or Opus-1M session. Pinning `model: fable` in frontmatter is avoided on purpose: a subagent spawn that explicitly requests Fable can be rejected by an availableModels policy, account access, or an older Claude Code version, whereas `inherit` honors the user's choice. Long research runs still need file-system persistence (`log.md`, `state.md`, `attempts/`) because very long runs may compress earlier history; the disk is always the source of truth.
